...
even if they have the same name. The built-in exceptions can be generated by the interpreter or built-in functions.
...
-----------------------------------------
An AttributeError is raised when an attribute reference or assignment fails such as when a 
...
exception EOFError
------------------------------
An EOFError is raised when built-in functions like input() hits an end-of-file condition (EOF) 
without reading any data. The file methods like readline() return an empty string when they hit EOF.
...
-----------------------------------
A FloatingPointError is raised when a floating point operation fails. This exception is always defined, 
but can only be raised when Python is configured with the–with-fpectl option, 
...
-----------------------------
This exception directly inherits from BaseException instead of Exception since it is technically not an error. 
...
--------------------------
An ImportError is raised when the import statement is unable to load a module or when the “from list” 
in from … import has a name that cannot be found.
...
--------------------------------------------
This is the subclass of ImportError which is raised by import when a module could not be found. 
...
-------------------------------------
This exception is derived from RuntimeError. Abstract methods in user defined classed should raise this 
...
------------------------------
The OSError exception is raised when a system function returns a system-related error, including I/O 
failures such as “file not found” or “disk full” errors.
...
Integers raise MemoryError instead of OverflowError. OverflowError is sometimes raised for 
integers that are outside a required range. Floating point operations are not checked because of the 
...
----------------------------------
The RecursionError is derived from the RuntimeError. This exception is raised when the interpreter 
detects that the maximum recursion depth is exceeded.

exception ReferenceError
--------------------------------
The ReferenceError is raised when a weak reference proxy is used to access an attribute of the referent 
...
The StopIteration error is raised by built-in function next() and an iterator‘s __next__() method to signal 
that all items are produced by the iterator.
...
-----------------------------------
The SyntaxError is raised when the parser encounters a syntax error. A syntax error may occur in an 
import statement or while calling the built-in functions exec() or eval(), or when reading the initial 
script or standard input.
...
---------------------------------
The SystemError is raised when the interpreter finds an internal error. The associated value is a string 
indicating what went wrong.

exception SystemExit
----------------------------
The SystemExit is raised when sys.exit() function is called. A call to sys.exit() is translated into an 
exception to execute clean-up handlers (finally clauses of try statements) and to debug a script 
without running the risk of losing control.
...
----------------------------------
UnboundLocalError is a subclass of NameError which is raised when a reference is made to a local 
...
A ValueError is raised when a built-in operation or function receives an argument that has the right 
